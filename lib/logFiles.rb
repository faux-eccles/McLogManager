class LogFile
    ...
    def getFileName()
        return File::basename(@logLocation)
    end
    
    def getUsers()
        # Goes line by line through a given file taking out and stripping the line to the required information
        #       if it matches the requirements, it prays on the reliable logging standard by the Essentials Bukkit plugin
        
        # There is an obvious problem however, at the moment it only checks for the string "UUID of player"
        ...
        usernames = []                                                                    # This will hold the end data after it has been extracted
        for i in (0...@fileContent.length)
            if @fileContent[i].include? "UUID of player"    # This needs to be replaced by a regex.  
                username= @fileContent[i].split[7]                             # finds the username and IP in the given line and then snips away the excess data
                usernames.push(username)      # Add the data to the entries array in a csv style eg "127.0.0.1,username"
            end
        end
